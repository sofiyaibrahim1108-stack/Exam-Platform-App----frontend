...
const AdminSubjects = () => {
  ...
  const [stats, setStats] = useState({ total: 0, active: 0, theory: 0, practical: 0 });
  ...
  // Subject Types list
  const subjectTypes = ['Theory', 'Practical', 'Lab', 'Project'];
  ...
  const fetchSubjects = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to retrieve subjects list.');
    } finally {
      setLoading(false);
    }
  };

  // Load dropdown selectors data options
  const fetchDropdowns = async () => {
    try {
      const response = await api.get('/subjects/dropdowns');
      if (response.data && response.data.success) {
      ...
      console.error('Failed to load active departments/courses/semesters dropdown lists', error);
    }
  };

  useEffect(() => {
    fetchSubjects();
  }, [search, deptFilter, courseFilter, semFilter, statusFilter, sortBy, currentPage]);
  ...
  const onAddSubmit = async (data) => {
    const toastId = toast.loading('Registering Subject details...');
    try {
      ...
      toast.error(error.response?.data?.message || 'Subject registration failed.', { id: toastId });
    }
  };
  ...
  const handleEditClick = (sub) => {
    setSelectedSub(sub);
    setEditSelectedDept(sub.department?._id || '');
    setEditSelectedCourse(sub.course?._id || '');
    setValueEdit('name', sub.name);
    setValueEdit('code', sub.code);
    setValueEdit('department', sub.department?._id || '');
    setValueEdit('course', sub.course?._id || '');
    setValueEdit('semester', sub.semester?._id || '');
    setValueEdit('credits', sub.credits);
    setValueEdit('subjectType', sub.subjectType);
    setValueEdit('description', sub.description || '');
    setValueEdit('status', sub.status);
    setEditModalOpen(true);
  };

  // UPDATE Action
  const onEditSubmit = async (data) => {
    const toastId = toast.loading('Syncing subject records...');
    try {
      await api.put(`/subjects/${selectedSub._id}`, {
        name: data.name,
        code: data.code,
        department: data.department,
        course: data.course,
      ...
      toast.success('Subject details updated successfully.', { id: toastId });
      setEditModalOpen(false);
      fetchSubjects();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Subject modification failed.', { id: toastId });
    }
  };

  // HARD DELETE Action
  const handleDeleteConfirm = async () => {
    const toastId = toast.loading('Deleting subject record...');
    try {
      await api.delete(`/subjects/${selectedSub._id}`);
  ...
  const handleStatusToggle = async (sub) => {
    const toastId = toast.loading('Changing subject status...');
    try {
      const response = await api.patch(`/subjects/${sub._id}/status`);
  ...
  // Modal reactive filters for Course & Semester selectors
  const addFilteredCourses = courses.filter(course => course.department === addSelectedDept);
  const addFilteredSemesters = semesters.filter(sem => sem.course === addSelectedCourse);

  const editFilteredCourses = courses.filter(course => course.department === editSelectedDept);
  const editFilteredSemesters = semesters.filter(sem => sem.course === editSelectedCourse);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface p-6 rounded-[24px] border border-primary/5">
        <div>
          ...
          <p className="text-on-surface-variant text-xs mt-1">Configure degree subjects, credits allocations, syllabus content, and link terms.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { title: 'Total Subjects', count: stats.total, icon: 'auto_stories', color: 'text-primary' },
          { title: 'Active Subjects', count: stats.active, icon: 'check_circle', color: 'text-emerald-600' },
          { title: 'Theory Subjects', count: stats.theory, icon: 'history_edu', color: 'text-secondary' },
          { title: 'Practical Subjects', count: stats.practical, icon: 'biotech', color: 'text-amber-600' },
        ].map((card) => (
      ...
      <div className="glass-panel p-6 rounded-[24px] shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
            <span className="material-symbols-outlined text-primary/45 text-5xl">menu_book</span>
            <div className="space-y-1">
              <h3 className="text-lg font-bold text-primary">No Subjects Found</h3>
              <p className="text-on-surface-variant text-sm max-w-sm">No academic subjects mapped matching your filter search parameters.</p>
            </div>
          </div>
        ) : (
          // Data Table
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-primary/10 pb-4 text-xs font-mono font-semibold text-primary uppercase tracking-wider">
                  <th className="py-4 px-3">Subject Name</th>
              ...
              <tbody className="divide-y divide-primary/5 text-sm">
                {subjects.map((sub) => (
                  <tr key={sub._id} className="hover:bg-primary/5 transition-colors">
                    <td className="py-4 px-3 font-semibold text-primary">{sub.name}</td>
                    <td className="py-4 px-3 font-mono text-xs text-on-surface-variant">{sub.code}</td>
                    <td className="py-4 px-3">
                      {sub.department ? (
                        <div>
                          <p className="font-semibold text-on-surface leading-tight">{sub.department.name}</p>
                          <p className="text-[10px] text-on-surface-variant font-mono leading-none mt-0.5">{sub.department.code}</p>
                        </div>
                      ) : (
                    ...
                    <td className="py-4 px-3">
                      {sub.course ? (
                        <div>
                          <p className="font-semibold text-on-surface leading-tight">{sub.course.name}</p>
                          <p className="text-[10px] text-on-surface-variant font-mono leading-none mt-0.5">{sub.course.code}</p>
                        </div>
                      ) : (
                    ...
                    <td className="py-4 px-3 text-center font-mono text-on-surface">
                      {sub.semester ? `Semester ${sub.semester.semesterNumber}` : 'Unassigned'}
                    </td>
                    <td className="py-4 px-3 text-center font-mono font-bold text-on-surface">{sub.credits}</td>
                    <td className="py-4 px-3">
                      <span className="font-mono text-xs font-semibold px-2.5 py-0.5 bg-primary/10 text-primary rounded-full uppercase">
                        {sub.subjectType}
                      </span>
                    </td>
                    <td className="py-4 px-3 text-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${
                          sub.status === 'Active' ? 'bg-secondary/15 text-secondary' : 'bg-error/10 text-error'
                        } hover:scale-95 transition-transform`}
                      >
                        <span className={`w-1.5 h-1.5 rounded-full ${sub.status === 'Active' ? 'bg-secondary' : 'bg-error'}`}></span>
                        {sub.status}
                      </button>
                    </td>
      ...
      <AnimatePresence>
        {addModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-xl font-bold text-primary mb-2">Create Subject</h3>
              <p className="text-on-surface-variant text-xs mb-6">Provision a new academic subject linked to a specific term semester.</p>

              <form onSubmit={handleSubmitAdd(onAddSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Name */}
                  <div className="col-span-2">
                    <label className="block font-mono text-[10px] font-semibold text-on-surface-variant mb-1 uppercase px-1">Subject Name *</label>
                    <input
                      {...registerAdd('name', { required: 'Subject Name is required' })}
                  ...
                  {/* Code */}
                  <div>
                    <label className="block font-mono text-[10px] font-semibold text-on-surface-variant mb-1 uppercase px-1">Subject Code *</label>
                    <input
                      {...registerAdd('code', { required: 'Subject Code is required' })}
                      className="w-full input-underline py-2 focus:ring-0 text-base"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      defaultValue={3}
                      {...registerAdd('credits', { required: 'Credits weight is required', min: 0 })}
                      className="w-full input-underline py-2 focus:ring-0 text-base"
                      placeholder="e.g. 3"
                    />
                  ...
                  <div>
                    <label className="block font-mono text-[10px] font-semibold text-on-surface-variant mb-1 uppercase px-1">Subject Type *</label>
                    <select
                      {...registerAdd('subjectType', { required: 'Subject Type is required' })}
                      defaultValue="Theory"
                      className="w-full bg-transparent border-0 border-b-2 border-outline-variant py-2.5 text-base focus:ring-0 focus:border-primary outline-none cursor-pointer"
                    >
                    ...
                    {errorsAdd.subjectType && <span className="text-error text-xs block font-mono mt-1">{errorsAdd.subjectType.message}</span>}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Enter a brief syllabus summary or credit distribution notes..."
                    />
                  </div>
                </div>
        ...
        {editModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-surface-container-lowest max-w-xl w-full rounded-[24px] border border-primary/10 p-8 shadow-2xl z-50 max-h-[90vh] overflow-y-auto"
            >
              <h3 className="text-xl font-bold text-primary mb-2">Edit Subject</h3>
              <p className="text-on-surface-variant text-xs mb-6">Modify records and coordinates for this subject division.</p>
              ...
              <form onSubmit={handleSubmitEdit(onEditSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-2">
                    <label className="block font-mono text-[10px] font-semibold text-on-surface-variant mb-1 uppercase px-1">Subject Name *</label>
                    <input
                      {...registerEdit('name', { required: 'Subject Name is required' })}
                      className="w-full input-underline py-2 focus:ring-0 text-base"
                      placeholder="e.g. Design and Analysis of Algorithms"
                    />
                  ...
                  <div>
                    <label className="block font-mono text-[10px] font-semibold text-on-surface-variant mb-1 uppercase px-1">Subject Code *</label>
                    <input
                      {...registerEdit('code', { required: 'Subject Code is required' })}
                      className="w-full input-underline py-2 focus:ring-0 text-base"
                      placeholder="e.g. CS302"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      {...registerEdit('credits', { required: 'Credits weight is required', min: 0 })}
                      className="w-full input-underline py-2 focus:ring-0 text-base"
                      placeholder="e.g. 3"
                    />
                  ...
                  <div>
                    <label className="block font-mono text-[10px] font-semibold text-on-surface-variant mb-1 uppercase px-1">Subject Type *</label>
                    <select
                      {...registerEdit('subjectType', { required: 'Subject Type is required' })}
                      className="w-full bg-transparent border-0 border-b-2 border-outline-variant py-2.5 text-base focus:ring-0 focus:border-primary outline-none cursor-pointer"
                    >
                      <option value="">Select Type...</option>
                      {subjectTypes.map((type) => (
                    ...
                    {errorsEdit.subjectType && <span className="text-error text-xs block font-mono mt-1">{errorsEdit.subjectType.message}</span>}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      readOnly
                      value={selectedSub ? new Date(selectedSub.createdAt).toLocaleString() : ''}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      readOnly
                      value={selectedSub ? new Date(selectedSub.updatedAt).toLocaleString() : ''}
                      className="w-full border-0 border-b border-dashed border-outline-variant text-on-surface-variant/70 text-sm py-2 cursor-not-allowed bg-transparent outline-none"
                    />
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-primary">Delete Subject</h3>
                <p className="text-on-surface-variant text-sm">
                  Are you sure you want to delete the subject <strong>{selectedSub?.name} ({selectedSub?.code})</strong>? This action will perform a soft-delete and hide it from all listing indices.
                </p>
              </div>
            ...
            >
              <div className="space-y-8">
                ...
                <div className="flex justify-between items-center pb-4 border-b border-primary/10">
                  <h3 className="text-lg font-bold text-primary">Subject Details</h3>
                  <button
                    onClick={() => setDetailsDrawerOpen(false)}
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    <h4 className="text-xl font-bold text-primary">{selectedSub?.name}</h4>
                ...
                <div className="space-y-4 text-sm">
                  <div className="flex justify-between py-2 border-b border-primary/5">
                    <span className="font-mono text-xs text-on-surface-variant uppercase">Subject Type</span>
                    <span className="font-mono font-bold text-on-surface uppercase">{selectedSub?.subjectType}</span>
                  </div>

                  <div className="flex justify-between py-2 border-b border-primary/5">
                    <span className="font-mono text-xs text-on-surface-variant uppercase">Department</span>
                    <span className="font-semibold text-on-surface">
                      {selectedSub?.department ? selectedSub.department.name : 'Unassigned'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between py-2 border-b border-primary/5">
                    ...
                    <span className="font-semibold text-on-surface">
                      {selectedSub?.course ? `${selectedSub.course.name} (${selectedSub.course.code})` : 'Unassigned'}
                    </span>
                  </div>

                  <div className="flex justify-between py-2 border-b border-primary/5">
                    <span className="font-mono text-xs text-on-surface-variant uppercase">Semester Number</span>
                    <span className="font-mono font-bold text-on-surface">
                      {selectedSub?.semester ? `Semester ${selectedSub.semester.semesterNumber}` : 'Unassigned'}
                    </span>
                  </div>

                  <div className="flex justify-between py-2 border-b border-primary/5">
                    <span className="font-mono text-xs text-on-surface-variant uppercase">Credits weight</span>
                    <span className="font-mono font-bold text-on-surface">{selectedSub?.credits}</span>
                  </div>

                  <div className="flex justify-between py-2 border-b border-primary/5">
                    <span className="font-mono text-xs text-on-surface-variant uppercase">Clearance Status</span>
                    <span className={`px-2 py-0.5 rounded text-xs font-bold font-mono ${
                      selectedSub?.status === 'Active' ? 'bg-secondary/15 text-secondary' : 'bg-error/10 text-error'
                    }`}>
              ...
              <div className="pt-6 border-t border-primary/10 mt-8 flex justify-between items-center text-sm font-mono text-[9px] font-semibold text-on-surface-variant">
                <span>CREATED: {selectedSub?.createdAt ? new Date(selectedSub.createdAt).toLocaleDateString() : ''}</span>
                <span>UPDATED: {selectedSub?.updatedAt ? new Date(selectedSub.updatedAt).toLocaleDateString() : ''}</span>
              </div>
            </motion.div>
          </div>
        )}
